...
1. **Dignite.Paperbase.Abstractions（扩展契约层）**
   ...
   - **内容**：
     - 文档类型注册声明：`DocumentTypeDefinition`、`DocumentTypeOptions`（仅作 Host 启动声明入口，由 `HostDocumentTypeDataSeedContributor` 翻译落库到 `DocumentType` 实体）
...
Dignite.Paperbase.Abstractions（扩展契约层，无其他 Paperbase 项目依赖）
    ├── DocumentTypeDefinition / DocumentTypeOptions（Host 启动声明入口，落库到 DocumentType / FieldDefinition 实体）
...
| `OcrConfidence` | OCR | OCR 平均置信度（受置信度门槛影响） |

`FileOrigin`（Owned Entity）含 `OriginalFileName` / `FileSize` / `ContentType` / `ContentHash` 等上传时元数据。无独立"Filename / Size / Format"系统字段——读路径直接 `d.FileOrigin.OriginalFileName` 等。
...
类型绑定字段必须挂在某个文档类型下，按谁定义分两层。**两层 mutually exclusive ——按 `Document.TenantId` 决定本文档跑哪层字段定义，不混合**：

| 层级 | 谁定义 | 作用域（仅对……生效） | 例子 |
|------|-------|------------------|------|
| **Host 字段** | Host 部署者 | **Host 文档**（`Document.TenantId IS NULL`），即 Host 部署者自己上传的文档 | 例：Host 部署者运维内部文档时挂的"科室"字段 |
| **租户字段** | 租户（per-tenant）| **该租户文档**（`Document.TenantId = current`） | 例：租户在 Host "合同" type 下加"甲方/乙方/合同金额"；或在自定义"案件卷宗" type 下加"当事人/案由"字段 |

**关键约束（解读 X）**：

- Host 字段**不会自动作用于租户文档** —— 多租户 SaaS 场景下，业务字段全部由租户自配（与"Paperbase Core 不预置业务字段"一致）
- 同名字段跨层允许（Host "amount" + 租户 "amount" 不冲突，因为它们作用在不同 Document.TenantId 的文档上）

**(B) 机制本质**：Paperbase 提供"按 schema 抽取"的通用引擎——Host 或租户配 schema，引擎按所属层执行抽取。Paperbase Core **不预置任何业务字段定义**（合同金额 / 发票号 / 税额等不写死）。

**实现形态（字段架构 v2）**：

- 单一 `FieldDefinition` 实体承载两层（`TenantId IS NULL` = Host 字段定义；`TenantId != null` = 租户字段定义），唯一索引 `(TenantId, DocumentTypeCode, Name)`
- Host 部署者通过 `Configure<DocumentTypeOptions>(...)` + `AddField(...)` 声明 schema；`HostDocumentTypeDataSeedContributor` 在 host 启动时 upsert 落库
- 租户通过 `IFieldDefinitionAppService` CRUD 自己的字段（仅 `TenantId = CurrentTenant.Id` 行，不能改 Host 层）
- 单一 `FieldExtractionEventHandler` 订阅 `DocumentClassifiedEto`，按 `Document.TenantId` 精确匹配一层字段定义 → 一次 LLM 调用 → 写入 `Document.ExtractedFields`（SQL Server 2025 native `json` 列；EF Core 10 ValueConverter 中转 `Dictionary<string, JsonElement>`）
- 抽取完成统一发布 `FieldsExtractedEto`（薄载荷含 `FieldCount`；下游可按事件载荷 `TenantId` 区分场景）
- 详细判定 + EF Core / SQL Server 2025 选型理由：`plans/synthetic-leaping-kite.md`
...
| `FieldsExtractedEto` | 字段抽取完成（载荷分两段：`HostFieldCount` + `TenantFieldCount`） | 否 |
