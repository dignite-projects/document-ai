...
| 层级 | 谁定义 | 作用域 | 例子 / 说明 |
|------|-------|------|------------|
| **Host 部署层** | Host admin（`CurrentTenant.Id IS NULL`）| 仅 Host 自身文档（`Document.TenantId IS NULL`）使用 | 例：Host 自己运维内部文档需要的"合同 / 发票 / 报告 / 通用文档" |
| **租户层** | 租户 admin（per-tenant）| 仅该租户文档（`Document.TenantId = current`）使用 | 例：某律所私有"案件卷宗"类型；某医院私有"会诊记录"类型 |
...
- 上传时 Paperbase 自动用 LLM 跑分类 prompt → 在该 Document 所属层（按 `Document.TenantId` 精确匹配，`IDocumentTypeRepository.GetByTenantAsync`）内归类
...
| `OcrConfidence` | OCR | OCR 平均置信度（informational 质量指标，随出口事件透出，不门控 Ready） |
...
| 层级 | 谁定义 | 作用域（仅对……生效） | 例子 |
|------|-------|------------------|------|
| **Host 字段** | Host admin（`CurrentTenant.Id IS NULL`）| **Host 文档**（`Document.TenantId IS NULL`），即 Host 自己上传的文档 | 例：Host 在自管的"合同" type 下加"科室 / 内部合同号"字段 |
| **租户字段** | 租户 admin（per-tenant）| **该租户文档**（`Document.TenantId = current`） | 例：律所租户在自管的"案件卷宗" type 下加"当事人 / 案由"字段 |
...
- 跨层无 union：admin 视图、LLM 分类候选集、字段抽取，三处都按 `Document.TenantId` 精确匹配一层（`GetByTenantAsync` / `GetForExtractionAsync`）
...
- **OCR 置信度不再设门槛**（#196）：OCR 平均置信度预测不了真实质量（整页扫歪 / 模糊 / 版式乱未必反映在平均分），曾经的 host 默认 + per-tenant 覆盖配置一并移除；`OcrConfidence` 仍随 `OCRCompletedEto` / `DocumentReadyEto` 透出，作 informational 指标供下游次级 quality gating，不参与 Paperbase 内部门控
