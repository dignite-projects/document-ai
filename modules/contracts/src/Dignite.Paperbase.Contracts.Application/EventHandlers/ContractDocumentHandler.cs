    ...
    ITransientDependency
{
    ...
    private readonly PaperbaseContractsTelemetryRecorder _telemetry;
    ...
    public ContractDocumentHandler(
        ...
        PaperbaseContractsTelemetryRecorder telemetry,
    ...
    public virtual async Task HandleEventAsync(DocumentClassifiedEto eventData)
    {
        if (string.IsNullOrEmpty(eventData.DocumentTypeCode) ||
            !eventData.DocumentTypeCode.StartsWith(PaperbaseContractsDocumentTypes.Prefix))
        {
            return;
        }
