...
public class Document : FullAuditedAggregateRoot<Guid>, IMultiTenant
{
    ...
    internal void SetMarkdown(string markdown)
    {
        if (!string.IsNullOrEmpty(Markdown))
            throw new BusinessException(DocumentAIErrorCodes.Document.MarkdownIsImmutable);
    ...
    internal void SetTitle(string? title)
    {
        if (!string.IsNullOrEmpty(Title))
            throw new BusinessException(DocumentAIErrorCodes.Document.TitleIsImmutable);
