...
public class ExportTemplate : FullAuditedAggregateRoot<Guid>, IMultiTenant
{
    ...
    private void SetColumns(IReadOnlyList<ExportColumn> columns)
    {
        ...
        if (columns.Count == 0)
        {
            throw new BusinessException(DocumentAIErrorCodes.Export.TemplateRequiresColumn);
        }

        if (columns.Count > ExportTemplateConsts.MaxColumnCount)
        {
            throw new BusinessException(DocumentAIErrorCodes.Export.TemplateTooManyColumns)
        ...
        if (duplicate != null)
        {
            throw new BusinessException(DocumentAIErrorCodes.Export.TemplateDuplicateField)
    ...
    private static string ValidateName(string name)
    {
        ...
        {
            throw new BusinessException(DocumentAIErrorCodes.Export.InvalidTemplateName)
                .WithData("name", name);
        }
