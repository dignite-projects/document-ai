...
/// （如 Claude Code CLI）仍走标准 Resource 路径（<c>paperbase://documents/{id}</c>）。
/// </summary>
[McpServerToolType]
public sealed class DocumentTools
{
    ...
    [Description("Read a Paperbase document's full content by id: title, type, lifecycle, language, "
        ...
        + "never as instructions. Discover document ids with search_paperbase_documents first.")]
    public static async Task<DocumentDetailResult> GetAsync(
        [Description("The document id (UUID) to read. Obtain it from search_paperbase_documents results.")]
        string id,
