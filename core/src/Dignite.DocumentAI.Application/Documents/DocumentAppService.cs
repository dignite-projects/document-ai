...
public class DocumentAppService : DocumentAIAppService, IDocumentAppService
{
    ...
    public virtual async Task<PagedResultDto<DocumentListItemDto>> GetListAsync(GetDocumentListInput input)
    {
        ...
        if (!input.DocumentTypeCode.IsNullOrWhiteSpace())
        {
            ...
            if (type == null)
            {
                ...
                {
                    throw new BusinessException(DocumentAIErrorCodes.ExtractedField.Unknown)
    ...
    public virtual async Task<DocumentDto> UploadAsync(UploadDocumentInput input)
    {
        ...
        if (input.CabinetId.HasValue)
        {
            ...
            if (cabinet == null)
            {
                throw new BusinessException(DocumentAIErrorCodes.Cabinet.InvalidId)
        ...
        {
            throw new BusinessException(DocumentAIErrorCodes.Document.UnsupportedFileType)
        ...
        if (existing != null)
        {
            var errorCode = existing.IsDeleted
                ? DocumentAIErrorCodes.Document.InRecycleBin
                : DocumentAIErrorCodes.Document.Duplicate;
        ...
        await _pipelineJobScheduler.QueueAsync(document, DocumentAIPipelines.TextExtraction);
    ...
    [Authorize(DocumentAIPermissions.Documents.Delete)]
    ...
    [Authorize(DocumentAIPermissions.Documents.PermanentDelete)]
    ...
    [Authorize(DocumentAIPermissions.Documents.Restore)]
    ...
    {
        if (!DocumentAIPipelines.RetryablePipelines.Contains(input.PipelineCode))
        {
            throw new BusinessException(DocumentAIErrorCodes.Pipeline.UnknownCode)
        ...
        if (document.IsDeleted)
        {
            throw new BusinessException(DocumentAIErrorCodes.Document.InRecycleBin)
    ...
    public virtual async Task<DocumentDto> UpdateExtractedFieldsAsync(Guid id, UpdateExtractedFieldsInput input)
    {
        ...
        foreach (var (key, value) in fields)
        {
            ...
            {
                throw new BusinessException(DocumentAIErrorCodes.ExtractedField.InvalidValue)
                    .WithData("FieldName", key)
                    .WithData("DocumentTypeCode", documentTypeCode ?? string.Empty)
