...
public class DocumentPipelineRunAccessorTests
    : DocumentAIApplicationTestBase<DocumentPipelineRunAccessorTestModule>
{
    ...
    public async Task BeginOrStart_Uses_Pending_Run_For_Expected_Pipeline_When_JobArgs_RunId_Belongs_To_Another_Pipeline()
    {
        ...
        var textExtractionRun = await _pipelineRunManager.QueueAsync(document, DocumentAIPipelines.TextExtraction);
        var classificationRun = await _pipelineRunManager.QueueAsync(document, DocumentAIPipelines.Classification);
    ...
    public async Task BeginOrStart_Creates_Replacement_Run_With_JobArgs_RunId_When_Run_Is_Missing()
    {
        ...
        actualRun.PipelineCode.ShouldBe(DocumentAIPipelines.Classification);
