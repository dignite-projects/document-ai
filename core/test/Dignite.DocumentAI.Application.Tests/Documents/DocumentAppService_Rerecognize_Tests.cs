    ...
    : DocumentAIApplicationTestBase<DocumentAppServiceRerecognizeTestModule>
{
    ...
    public async Task RerecognizeAsync_Requeues_Classification_When_Already_Succeeded()
    {
        ...
        var newRun = await _runRepository.FindLatestByDocumentAndCodeAsync(doc.Id, DocumentAIPipelines.Classification);
    ...
    public async Task RerecognizeAsync_Throws_When_No_Markdown()
    {
        ...
        ex.Code.ShouldBe(DocumentAIErrorCodes.Document.NotTextExtracted);
        await _backgroundJobManager.DidNotReceive().EnqueueAsync(
            Arg.Any<DocumentClassificationJobArgs>(),
            Arg.Any<BackgroundJobPriority>(),
            Arg.Any<TimeSpan?>());
