...
public class DocumentPipelineRunManagerTests : DocumentAIDomainTestBase<DocumentAIDomainTestModule>
{
    ...
    public async Task Queue_Creates_Pending_Run_Before_BackgroundJob_Execution()
    {
        ...
        var run = await _manager.QueueAsync(doc, DocumentAIPipelines.TextExtraction);

        run.Status.ShouldBe(PipelineRunStatus.Pending);
        run.AttemptNumber.ShouldBe(1);
        (await _runRepo.FindLatestByDocumentAndCodeAsync(doc.Id, DocumentAIPipelines.TextExtraction))
            .ShouldBe(run);
    ...
    public async Task Retry_Increments_AttemptNumber_And_Latest_Run_State_Wins()
    {
        ...
        // Attempt 1 — fail
        var run1 = await _manager.StartAsync(doc, DocumentAIPipelines.TextExtraction);
        ...
        var run2 = await _manager.StartAsync(doc, DocumentAIPipelines.TextExtraction);
