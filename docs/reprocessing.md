...
**Judgment stays with the operator.** Config changes do **not** cascade automatically — Paperbase never guesses whether you meant a change to apply to existing documents. You trigger reprocessing explicitly, choose its scope, and accept its cost. This page covers the feature; for orchestration code see `core/src/Dignite.Paperbase.Application/Documents/Pipelines/Reprocessing/` and `.../FieldExtraction/`.
...
- **Lifecycle-neutral**: `field-extraction` is deliberately **not** a key pipeline (`PaperbasePipelines.KeyPipelines` = `{ text-extraction, classification }`), so re-extracting fields never flips an already-`Ready` document back to `Processing`. The `DocumentPipelineRun` row exists for observability and retry only.
...
| Permission | Gates |
|---|---|
| `Paperbase.Documents.Reprocessing.FieldExtraction` | batch field re-extraction (preview + trigger) |
| `Paperbase.Documents.Reprocessing.Reclassification` | batch reclassification (preview + trigger) |
| `Paperbase.Documents.ConfirmClassification` | single-document *Re-extract fields* (operator-level, same as *Re-recognize*) |

## REST endpoints

| Method | Route | Purpose |
|---|---|---|
| `GET` | `/api/paperbase/document-reprocessing/field-extraction/preview?documentTypeId=` | affected count + the type's current field names |
| `POST` | `/api/paperbase/document-reprocessing/field-extraction` | start batch field re-extraction |
| `POST` | `/api/paperbase/document-reprocessing/reclassification/preview` | affected count for a scope |
| `POST` | `/api/paperbase/document-reprocessing/reclassification` | start batch reclassification |
| `POST` | `/api/paperbase/documents/{id}/reextract-fields` | single-document re-extract fields |
