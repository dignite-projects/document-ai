...
Export templates are Document AI's **file-based egress** — the "last mile" to downstream systems that have no API and can only ingest files (Yayoi, freee, mid-market Yonyou, and similar accounting packages that import CSV). They sit alongside the four programmatic egresses (REST / MCP / EventBus / Webhook) but serve a different audience: a human who downloads a file and imports it into another system.

A template is **per-tenant configuration** following the same two-layer model as `DocumentType` and `FieldDefinition`. Document AI ships **no built-in templates** — there is no industry vertical schema baked in. The export engine only does **field projection → rename → ordering → serialization**, with **zero business transformation** (no tax calculation, no account-code mapping, no currency conversion). Business formats are something tenants *compose* out of a template; Document AI enables them rather than doing them.
...
- **Fixed system fields** — always emitted, not configurable: `LifecycleStatus`, `ReviewStatus`, `Title`. These are Document AI's stable metadata contract; tenants don't configure them the way they configure business fields.
...
> EventBus-triggered export is **not** offered: a subscriber that already consumes the EventBus has the structured data — having Document AI generate a file and hand it back closes no loop.
...
- Permissions: managing templates needs `Document AI.Documents.Templates.*`; running an export needs `Document AI.Documents.Export`.

## Example: composing a freee-style import CSV

Document AI ships nothing freee-specific. You compose the format from your own type-bound fields.
...
The same mechanism produces a Yayoi journal layout, a Yonyou voucher CSV, or any other ingest format: define the fields, map them to the target column names, pick the order. If a target format needs a value Document AI doesn't capture, add a `FieldDefinition` for it — the channel still doesn't *know* what freee is, it just lets you describe the shape.
