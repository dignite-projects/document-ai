...
[DependsOn(
    ...
    typeof(ExtractMcpModule),          // MCP exit adapter, parallel to the HttpApi REST exit.
    typeof(ExtractApplicationModule),
    typeof(ExtractEntityFrameworkCoreModule),
...
public class ExtractHostModule : AbpModule
{
    ...
    private void ConfigureDistributedEventBus()
    {
        Configure<AbpDistributedEventBusOptions>(options =>
        {
            ...
            {
                config.UseDbContext<Data.ExtractHostDbContext>();
            });
    ...
    {
        context.Services.AddExtractHealthChecks();
    }
    ...
    private void ConfigureLocalization()
    {
        ...
        {
            options.Resources
                .Add<ExtractHostResource>("en")
                .AddBaseTypes(typeof(ExtractResource))
                .AddVirtualJson("/Localization/ExtractHost");

            options.DefaultResourceType = typeof(ExtractHostResource);
    ...
    private void ConfigureAutoApiControllers()
    {
        ...
        {
            options.ConventionalControllers.Create(typeof(ExtractHostModule).Assembly);
        });
    }
    ...
    private void ConfigureVirtualFiles(IWebHostEnvironment hostingEnvironment)
    {
        ...
        {
            options.FileSets.AddEmbedded<ExtractHostModule>();
            if (hostingEnvironment.IsDevelopment())
            {
                /* Using physical files in development, so we don't need to recompile on changes */
                options.FileSets.ReplaceEmbeddedByPhysical<ExtractHostModule>(hostingEnvironment.ContentRootPath);
            }
        });
    }
    ...
    {
        context.Services.AddAbpDbContext<Data.ExtractHostDbContext>(options =>
        {
    ...
    private void ConfigureOpenTelemetry(ServiceConfigurationContext context, IConfiguration configuration)
    {
        ...
        var serviceVersion = typeof(ExtractHostModule).Assembly.GetName().Version?.ToString() ?? "1.0.0";
