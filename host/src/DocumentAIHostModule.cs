...
using Dignite.DocumentAI.Host.Data;
using Dignite.DocumentAI.Host.HealthChecks;
...
public class DocumentAIHostModule : AbpModule
{
    ...
    public override void PreConfigureServices(ServiceConfigurationContext context)
    {
        ...
        PreConfigure<OpenIddictBuilder>(builder =>
        {
            ...
            {
                // Single audience for the whole DocumentAI API. REST and MCP are two exit adapters
                // over the same OpenIddict-Bearer resource — every token's aud is "DocumentAI",
                // regardless of how it was obtained (manual static Bearer or MCP Guided OAuth).
                options.AddAudiences("DocumentAI");
        ...
        // DocumentAI is a SINGLE protected resource (audience "DocumentAI"); REST and MCP are just two
        ...
        // single-resource server. If DocumentAI ever becomes multi-resource, revisit (re-enable the
    ...
    private void ConfigureDistributedEventBus()
    {
        Configure<AbpDistributedEventBusOptions>(options =>
        {
            ...
            {
                config.UseDbContext<Data.DocumentAIHostDbContext>();
            });
    ...
    {
        context.Services.AddDocumentAIHealthChecks();
    }
    ...
    private void ConfigureMcpAuthentication(ServiceConfigurationContext context)
    {
        ...
        context.Services.AddAuthentication().AddMcp(options =>
        {
            options.ResourceMetadata = new ProtectedResourceMetadata
            {
                ...
                // (see the rationale there), so the issued token's aud stays "DocumentAI". Set
    ...
    private void ConfigureLocalization()
    {
        ...
        {
            options.Resources
                .Add<DocumentAIHostResource>("en")
                .AddBaseTypes(typeof(DocumentAIResource))
                .AddVirtualJson("/Localization/DocumentAIHost");

            options.DefaultResourceType = typeof(DocumentAIHostResource);
    ...
    private void ConfigureAutoApiControllers()
    {
        ...
        {
            options.ConventionalControllers.Create(typeof(DocumentAIHostModule).Assembly);
        });
    }
    ...
    private void ConfigureSwagger(IServiceCollection services, IConfiguration configuration)
    {
        services.AddAbpSwaggerGenWithOAuth(
            ...
            {
                {"DocumentAI", "DocumentAI API"}
            },
            options =>
            {
                options.SwaggerDoc("v1", new OpenApiInfo { Title = "DocumentAI API", Version = "v1" });
    ...
    {
        context.Services.AddDataProtection().SetApplicationName("DocumentAI");
    }
    ...
    private void ConfigureVirtualFiles(IWebHostEnvironment hostingEnvironment)
    {
        ...
        {
            options.FileSets.AddEmbedded<DocumentAIHostModule>();
            if (hostingEnvironment.IsDevelopment())
            {
                /* Using physical files in development, so we don't need to recompile on changes */
                options.FileSets.ReplaceEmbeddedByPhysical<DocumentAIHostModule>(hostingEnvironment.ContentRootPath);
            }
        });
    }
    ...
    {
        context.Services.AddAbpDbContext<Data.DocumentAIHostDbContext>(options =>
        {
    ...
    private void ConfigureAI(ServiceConfigurationContext context, IConfiguration configuration)
    {
        ...
        var endpoint = configuration["DocumentAI:Endpoint"];
        var apiKey = configuration["DocumentAI:ApiKey"];
        var chatModelId = configuration["DocumentAI:ChatModelId"];
            ...
            || string.IsNullOrWhiteSpace(chatModelId))
        {
            throw new AbpException(
                "DocumentAI requires an LLM provider before it can start: document classification and " +
                "field extraction have no non-LLM fallback. Set DocumentAI:Endpoint, DocumentAI:ApiKey, " +
                "and DocumentAI:ChatModelId in host/src/appsettings.Development.json (git-ignored), " +
        ...
        // [FromKeyedServices(DocumentAIConsts.TitleGeneratorChatClientKey)]. Falls back
        ...
        var titleGeneratorModelId = configuration["DocumentAI:TitleGeneratorModelId"]
            ?? chatModelId;
        ...
        var structuredModelId = configuration["DocumentAI:StructuredModelId"]
            ?? chatModelId;
        context.Services.AddKeyedChatClient(
        ...
        var visionOcrModelId = configuration["DocumentAI:VisionOcrModelId"];
        if (string.IsNullOrWhiteSpace(visionOcrModelId))
        {
            throw new AbpException(
                "VisionLlm is the configured OCR provider but DocumentAI:VisionOcrModelId is not set. " +
    ...
    private void ConfigureOpenTelemetry(ServiceConfigurationContext context, IConfiguration configuration)
    {
        ...
        var serviceVersion = typeof(DocumentAIHostModule).Assembly.GetName().Version?.ToString() ?? "1.0.0";

        var otel = context.Services.AddOpenTelemetry();

        otel.ConfigureResource(resource => resource
            .AddService(serviceName: "Dignite.DocumentAI", serviceVersion: serviceVersion));
    ...
    public override void OnApplicationInitialization(ApplicationInitializationContext context)
    {
        ...
        {
            options.SwaggerEndpoint("/swagger/v1/swagger.json", "DocumentAI API");
