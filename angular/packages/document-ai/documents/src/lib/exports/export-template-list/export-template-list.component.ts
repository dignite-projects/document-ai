...
@Component({
  ...
  providers: [
    ListService,
    {
      ...
      useValue: DOCUMENT_AI_TABLES.ExportTemplates,
    },
  ],
...
export class ExportTemplateListComponent implements OnInit {
  ...
  readonly canManage = this.permissionService.getGrantedPolicy(
    DOCUMENT_AI_PERMISSIONS.Documents.Templates.Default,
  );
  readonly canExport = this.permissionService.getGrantedPolicy(
    DOCUMENT_AI_PERMISSIONS.Documents.Export,
  );
  ...
  constructor() {
    configureEntityTable<ExportTemplateDto>(this.extensions, DOCUMENT_AI_TABLES.ExportTemplates, [
