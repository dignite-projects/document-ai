...
@Component({
  ...
  providers: [
    ListService,
    {
      ...
      useValue: DOCUMENT_AI_TABLES.DocumentRecycleBin,
    },
  ],
...
export class DocumentRecycleBinComponent implements OnInit {
  ...
  readonly canRestore = this.permissionService.getGrantedPolicy(
    DOCUMENT_AI_PERMISSIONS.Documents.Restore,
  );
  readonly canPermanentDelete = this.permissionService.getGrantedPolicy(
    DOCUMENT_AI_PERMISSIONS.Documents.PermanentDelete,
  );
  readonly hasRecycleActions = this.canRestore || this.canPermanentDelete;

  constructor() {
    configureEntityTable<DocumentListItemDto>(this.extensions, DOCUMENT_AI_TABLES.DocumentRecycleBin, [
