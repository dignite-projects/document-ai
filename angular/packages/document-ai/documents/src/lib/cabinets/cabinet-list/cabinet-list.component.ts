...
@Component({
  ...
  providers: [
    ListService,
    {
      ...
      useValue: DOCUMENT_AI_TABLES.Cabinets,
    },
  ],
...
export class CabinetListComponent implements OnInit {
  ...
  readonly canCreate = this.permissionService.getGrantedPolicy(DOCUMENT_AI_PERMISSIONS.Cabinets.Create);
  readonly canUpdate = this.permissionService.getGrantedPolicy(DOCUMENT_AI_PERMISSIONS.Cabinets.Update);
  readonly canDelete = this.permissionService.getGrantedPolicy(DOCUMENT_AI_PERMISSIONS.Cabinets.Delete);
  ...
  constructor() {
    configureEntityTable<CabinetDto>(this.extensions, DOCUMENT_AI_TABLES.Cabinets, [
